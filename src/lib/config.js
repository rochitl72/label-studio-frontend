...
//      start.
//
// Input:   window.API_BASE_PATH (set by public/config.js, editable on the
//          server without a rebuild — see that file), falling back to the
//          VITE_API_BASE_URL build-time environment variable, falling back
//          to same-origin.
// Process: normalises it (strips a trailing slash).
// Output:  API_ORIGIN and API_BASE (every REST call prefixes API_BASE).
const RAW = (
  (typeof window !== "undefined" && window.API_BASE_PATH) ||
